...
'＂'=>'"'); 
    ...
    public function stuCommonCertification(){
        ...
        if($stuinfo[0][birthday] == '' || $stuinfo[0][birthday] == '0000-00-00 00:00:00'){
            ...
            $birthday=date('M.jS,Y',strtotime($stuinfo[0][birthday]));
        }
        ...
        if($classinfo[0][majore] == ''){
            $majore='_____________________________________________';
        }else{
        ...
        $this->assign('current_time',date('M.jS,Y',time()));
    ...
    public function downScore(){
        ...
        $student=$classstudent->Field("name,studentname,majore,ename,year,student")->where("student ='".$id."'")->find();
        $Score = D("Score");
		$map['susername']=$id;
		$map['isvisible']=1;
        ...
        if(substr($student['name'],7,3) =='2+2'){
            ...
            $actSheet->setCellValue('A4','Name:'.$student['ename'].'   Student No.:'.$student['student'].'  Major: '.$student['majore'].'       Department: DAFLS');
            ...
            $actSheet->setCellValue('A2',"No:".$student['student']."       "."Name:".$student['ename']."        "."Major:".$student[majore]."        "."Department:DAFLS"."        "."Higher National Diploma Program");
    ...
    public function downCertification(){
        ...
        $PHPWord->addFontStyle('rStyle', array('bold'=>true,'name'=>'Arial','size'=>'16'));
        ...
        $section->addText('HND'.' '.'在读证明', 'rStyle', 'pStyle');
        $section->addTextBreak(2);
        $textrun = $section->createTextRun();
        $textrun->addText("　　"."兹证明",'cStyle');
        $textrun->addText($name,array('bold'=>true,'name'=>'Arial','size'=>'12'));
        $textrun->addText("同学,".$zsex."，生于".$year."年".$month."月".$day."日，于".$enrollyear."年9月进入南京大学大学外语部英国高等教育文凭（Higher National Diploma 简称HND）项目学习。（该项目引进自苏格兰学历管理委员会）。",'cStyle');
        $section->addTextBreak(1);
        $section->addText("　　"."该学生目前为该项目".$major."专业".$current_grade."年级学生。",'cStyle');
        $section->addTextBreak(1);
        $section->addText('特此证明','cStyle');
        $section->addTextBreak(2);
        $section->addText('南京大学 大学外语部','cStyle','YStyle');
        $section->addText($nowyear."年".$nowmonth."月".$nowday."日",'cStyle','YStyle');
        $section->addTextBreak(3);
        $section->addText("$current_time",array('name'=>'Times New Roman','size'=>'12'));
        $section->addText('HND Student Status Certificate',array('bold'=>true,'name'=>'Times New Roman','size'=>'16'), 'pStyle');
        $section->addTextBreak(3);
        $textrun = $section->createTextRun();
        $textrun->addText("This is to certify that student ",array('name'=>'Times New Roman','size'=>'12'));
        $textrun->addText($ename,array('bold'=>true,'name'=>'Times New Roman','size'=>'12'));
